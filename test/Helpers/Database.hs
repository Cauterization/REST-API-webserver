...
import Control.Monad.State ( when, gets, modify )
...
import Database.Database
    ( DBError(EntityNotFound, AlreadyExists),
      EntityFilterParam(EFPInt),
      IsDatabase(..) )
...
import HKD.HKD ( Front, Create, Display, Delete )
import Helpers.Monad
    ( TestState(_tsDraftDB, _tsPictureDB, _tsCatDB, _tsTagDB,
                _tsAuthorDB, _tsUserDB, _tsGetFilters, _tsIDs),
      EMap,
      TestMonad,
      tsAuthorDB,
      tsCatDB,
...
instance IsDatabase TestDB where
  ...
  getFromDatabase ::
    forall q r.
    (ToRowOf TestDB q, FromRowOf TestDB r) =>
    () ->
    TestQuery ->
    q ->
    TestMonad [r]
