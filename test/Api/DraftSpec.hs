...
propPostsDraft dbD dbU draft = property $ conditions ==> do
    (Right (ResText t), st) <- runTest 
...
propPostsDraftWrongToken draft t dbU = property $ condition ==> do
...
propGetDrafts dbD e = property $ condition ==> do
  ...
  where
    ...
    ref = map (\(a, b) -> Entity (ID a) (fromDisplay @(Draft (Front Display)) b)) $ IM.toList $
...
propGetDraftsIsPaginated (unBigTDB -> dbD) e = property $ condition ==> do
    Right (ResJSON j) <- evalTest 
        ( withGetPath "drafts"
        . withToken (token u)) 
        ( withDatabase @Draft dbD 
...
propGetDraftsWithlimitOffset dbD e limit offset = property $ condition ==> do
    Right (ResJSON j) <- evalTest 
        ( withGetPath "drafts"
        . withToken (token u)
        . withLimit limit
  ...
  where
    condition = length (IM.toList dbD) < testPaginationConstant
    ref = paginate $ map (\(a, b) -> Entity (ID a) (fromDisplay @(Draft (Front Display)) b)) $ IM.toList $
        IM.filter ((== ID userID) . entityID . user . entity . author . unDraft) dbD
    Entity (ID userID) u = e 
    dbU =  IM.singleton userID u
...
propGetDraft dbD (ID draftID) u = property $ conditions ==> do
    Right (ResJSON j) <- evalTest 
...
propGetDraftDoesntExists dbD (ID draftID) e = property $ conditions ==> do
    Left err <- evalTest
        ( withGetPath ("drafts/" <> T.show draftID)
        . withToken (token u)) 
        ( withDatabase @Draft dbD 
...
propGetDraftWrongToken dbD (ID draftID) u wrongT = property $ conditions ==> do
    Left err <- evalTest 
        ( withGetPath ("drafts/" <> T.show draftID)
        . withToken wrongT) 
        ( withDatabase @Draft dbD 
...
propDeleteDraft dbD (ID draftID) u = property $ conditions ==> do
    st <- execTest
...
propDeleteDraftDoesntExists dbD (ID draftID) e = property $ conditions ==> do
    Left res <- evalTest
...
propDeleteDraftWrongToken dbD (ID draftID) u wrongT = property $ conditions ==> do
    Left res <- evalTest
        ( withDeletePath ("drafts/" <> T.show draftID) 
        . withToken wrongT)
        ( withDatabase @Draft dbD 
...
propPublishDraftDoesntExists dbD (ID draftID) e = property $ conditions ==> do
    Left err <- evalTest
        ( withPublishPath ("drafts/" <> T.show draftID)
        . withToken (token u))
        ( withDatabase @Draft dbD
