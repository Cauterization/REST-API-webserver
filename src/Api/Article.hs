...
getDrafts _ = do
    ...
    token <- getToken
    json =<< Database.getEntitiesWith @Article @(Front Display) [token]
        (<> " WHERE token = ? AND published = false")

getDraftbyID :: forall m.
    ( Application m
    , Database.ToRowOf (Database m) (Token, ID (Article (Front Display)))
    , Gettable m Article (Front Display)
    ) => Endpoint m 
getDraftbyID [dID] = do
    Logger.info "Attempt to get drafts"
    ...
    json =<< Database.getSingle =<< 
        Database.getEntitiesWith @Article @(Front Display) 
            (token, coerce @_ @(ID (Article (Front Display))) dID)
            (<> " WHERE token = ? AND id = ? AND published = false")
getDraftbyID _ = entityIDArityMissmatch "get draft"
...
publishDraft [draftID]= do
    ...
    text =<< Database.publish @(Entity Article) @m @Publish q
        (Entity (coerce draftID) 
            Article{author   = coerce entityID
                  , created  = now 
                  , title    = Nothing 
                  , content  = Nothing 
                  , category = Nothing 
